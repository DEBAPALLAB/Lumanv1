...
import { ArrowLeft, Calendar, CheckSquare, FileText, Sparkle, Sparkles } from "lucide-react";
import Link from "next/link";
import type { ReactNode } from "react";

/**
 * Shared chrome for every step of the authentication funnel.
 *
 * Both the "join an existing workspace" path (/org-login -> /login) and the
 * "create a new workspace" path (/org-register -> /register), plus the invite
 * path (/join), render inside this shell so the whole flow reads as one product.
 */

/* -------------------------------------------------------------------------- */
/*  Left side - premium info showcase of what Luman does                       */
/* -------------------------------------------------------------------------- */

function ShowcasePanel({ eyebrow }: { eyebrow: string }) {
  return (
    <aside className="hidden lg:flex lg:w-7/12 bg-[#FDFBF7] dark:bg-zinc-950 relative overflow-hidden flex-col justify-center p-16 xl:p-20 border-r-[4px] border-black">
            ...
            <div className="space-y-2 flex-grow">
              ...
              <div className="space-y-1.5 pt-1">
                <div className="flex items-center gap-2 text-[10px] font-black uppercase text-black/80">
                  <span className="w-4 h-4 border-2 border-black rounded-md bg-black flex items-center justify-center text-white text-[8px]">
                    ✓
                  </span>
                ...
                <div className="flex items-center gap-2 text-[10px] font-black uppercase text-black">
                  <span className="w-4 h-4 border-2 border-black rounded-md bg-white flex items-center justify-center font-black">
                    /
                  </span>
...
/* -------------------------------------------------------------------------- */
/*  Decorative doodles layered behind the form card                            */
/* -------------------------------------------------------------------------- */
  ...
  return (
    <>
      ...
      <div className="absolute top-0 left-0 w-24 h-24 overflow-hidden pointer-events-none opacity-40">
        <svg
          className="w-full h-full text-foreground/[0.04] stroke-current"
          viewBox="0 0 100 100"
          fill="none"
          aria-hidden="true"
        >
      ...
      <svg
        className="absolute top-6 right-6 w-8 h-8 text-[#FBBF24] fill-current animate-pulse"
        viewBox="0 0 24 24"
        aria-hidden="true"
      >
      ...
      <svg
        className="absolute bottom-28 left-6 w-6 h-6 text-[#FCD34D] fill-current animate-bounce"
        viewBox="0 0 24 24"
        aria-hidden="true"
      >
        <path d="M12 0L14.6 9.4L24 12L14.6 14.6L12 24L9.4 14.6L0 12L9.4 9.4L12 0Z" />
      ...
      <div className="absolute left-4 top-1/2 -translate-y-1/2 w-16 h-16 pointer-events-none opacity-30">
        <svg
          className="w-full h-full text-indigo-500/[0.08] dark:text-indigo-400/[0.08] stroke-current"
          viewBox="0 0 100 100"
          fill="none"
          strokeWidth="2"
          aria-hidden="true"
      ...
      <div className="absolute -right-2 bottom-4 w-12 h-40 pointer-events-none opacity-30 select-none hidden xl:block z-0">
        <svg
        ...
        >
          <path
            d="M10,20 L40,0 L50,15 L20,35 L50,55 L60,70 L30,90 L60,110 L70,125 L40,145 L70,165 L60,180"
            fill="#1E3A8A"
          />
...
export interface AuthShellProps {
  /** Badge copy on the showcase panel. */
  eyebrow?: string;
  /** Card headline. */
  title: string;
  /** Small-caps line under the headline, usually the step indicator. */
  subtitle: string;
  /** Where the floating back capsule points, and what it reads. */
  backHref?: string;
  backLabel?: string;
  /** The form body. */
  children: ReactNode;
  /** Rendered below a divider at the bottom of the card. */
  footer?: ReactNode;
  /** Widen the card for steps that need more room (e.g. the role builder). */
  wide?: boolean;
}
...
export function AuthShell({
  ...
  return (
    <div className="min-h-screen w-full bg-[#FDFBF7] dark:bg-zinc-950 flex flex-col lg:flex-row overflow-hidden font-sans relative">
      <ShowcasePanel eyebrow={eyebrow} />

      {/* Right side - the active form card */}
      <main className="w-full lg:w-5/12 flex items-center justify-center p-6 sm:p-12 relative z-10 min-h-screen">
        ...
        <div
          ...
          } border-[4px] border-black bg-[#FDFBF7] dark:bg-zinc-900 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] dark:shadow-[12px_12px_0px_0px_rgba(255,255,255,1)] rounded-[36px] overflow-hidden my-auto flex flex-col justify-between p-8 sm:p-10 min-h-[560px]`}
        >
          <CardDoodles />

          {/* Card Content Wrapper */}
          <div className="space-y-8 flex-grow flex flex-col justify-between relative z-10 pb-2">
            {/* Header Title Block */}
            <div className="space-y-4 text-center mt-4">
              <Link href="/" className="inline-flex items-center gap-2 group w-max mx-auto">
                ...
                <span className="text-sm font-black uppercase tracking-widest text-foreground">luman</span>
              </Link>

              <div className="space-y-2">
                <h1 className="text-2xl sm:text-3xl font-black uppercase tracking-tight text-foreground">{title}</h1>
                <p className="text-[10px] font-black tracking-wider text-muted-foreground uppercase">{subtitle}</p>
              </div>
            </div>
          ...
          {/* Secure Footer Flag */}
          <div className="relative z-10 mt-8 pt-4 border-t border-foreground/10 flex justify-between items-center text-[8px] font-black uppercase text-muted-foreground/60 select-none">
