...
export function ProfileBadge({ identity }: { identity: Identity }) {
  ...
  return (
    <div ref={rootRef} className="fixed left-4 top-4 z-[9100]">
      ...
      >
        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-[7px] border-[2px] border-black bg-[#FBBF24] text-[11px] font-bold text-black dark:border-stone-100">
          {initialsFor(identity)}
        </span>
        <span className="flex flex-col items-start leading-none">
          <span className="max-w-[120px] truncate text-[11.5px] font-bold text-black dark:text-stone-100">
            {displayName}
          </span>
          {identity.orgName && (
            <span className="max-w-[120px] truncate text-[9.5px] font-semibold text-black/40 dark:text-stone-100/40">
              {identity.orgName}
            </span>
          )}
        </span>
      ...
      {open && (
        ...
        >
          <div className="flex items-center gap-3 border-b-[2px] border-black/10 px-3.5 py-3 dark:border-stone-100/10">
            <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[10px] border-[2px] border-black bg-[#FBBF24] text-[14px] font-bold text-black dark:border-stone-100">
              {initialsFor(identity)}
            </span>
            <div className="min-w-0">
              <p className="truncate text-[13px] font-bold text-black dark:text-stone-100">{displayName}</p>
              {identity.email && (
                <p className="truncate text-[11px] text-black/45 dark:text-stone-100/45">{identity.email}</p>
              )}
            </div>
          </div>
            ...
            <Row
              ...
              value={
                <span className="inline-flex items-center gap-1">
                  <ShieldCheck className="h-3 w-3 text-black/40 dark:text-stone-100/40" strokeWidth={2.5} />
          ...
          <div className="border-t-[2px] border-black/10 p-1.5 dark:border-stone-100/10">
            <a
              href="/settings"
              className={cn(
                ...
                "dark:text-stone-100 dark:hover:bg-stone-100/[0.08]",
              )}
            >
              <Settings className="h-4 w-4 text-black/50 dark:text-stone-100/50" strokeWidth={2.25} />
              Settings
            </a>
            <button
              type="button"
...
function Row({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between gap-3">
      <span className="text-[10px] font-bold uppercase tracking-[0.08em] text-black/35 dark:text-stone-100/35">
        {label}
      </span>
      <span className="min-w-0 truncate font-semibold text-black dark:text-stone-100">{value}</span>
    </div>
  );
}
